...
class Uniform(BaseDistribution):
    ...
    def __init__(self, shape, low=-1.0, high=1.0):
        ...
        self.log_prob_val = -self.d * torch.log(self.high - self.low)
    ...
    def forward(self, num_samples=1, context=None):
        ...
        log_p = self.log_prob_val.sum() * torch.ones(num_samples, device=self.low.device)
        return z, log_p

    def log_prob(self, z, context=None):
        log_prob_val = self.log_prob_val.sum()
        log_p = log_prob_val * torch.ones(z.shape[0], device=z.device)
        # log_p = log_prob_val * torch.ones(z.shape[0])
        out_range = torch.logical_or(z < self.low.cuda(), z > self.high.cuda())
...
class GaussianDistribution(BaseDistribution):
    ...
    def forward(self, num_samples=1, context=None):
        ...
        q = Normal(self.loc, self.scale)
        log_p = q.log_prob(z).sum(1)
        return z, log_p
